/*
paras: starttime, stoptime, usetime, enable
Get usetime from settings
Register: 
  enable=True
  starttime = nowtime()
  stoptime = starttime + usetime
Check Always:
  if enable == True:
    If nowtime() > stoptime:
      enable = False
...
document.addEventListener("click", function(e) {
  ...
  if(e.target.id == "settings_submit"){
    var hour = document.getElementById("use_time_hour").value;
    var minute = document.getElementById("use_time_minute").value;
    var second = document.getElementById("use_time_second").value;
    
    chrome.storage.sync.set({usetimehour: hour}, function() {
      //alert('Hour ' + hour);
    });
    chrome.storage.sync.set({usetimeminute: minute}, function() {
      //alert('Minute ' + minute);
    });
    chrome.storage.sync.set({usetimesecond: second}, function() {
      //alert('Second ' + second);
    });
